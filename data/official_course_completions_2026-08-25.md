...
## Merge addendum (landed onto main, 2026-08-25)

This branch was landed onto a newer main that already contained the ST-1027–ST-1097
podcast block and the ST-1098–ST-1176 world-models block (1,175 resources, max id
ST-1176). The recipe above was followed: `npm run courses:complete` ran against the
live main catalog and added all 77 rows under their reserved ids **ST-1200–ST-1276**
(no videoId/url overlap with main; the ST-1177–ST-1199 gap is intentional), followed
by `npm run notes:apply`. `npm run series:apply` was skipped because it currently
fails on main for a pre-existing reason unrelated to this branch (the world-models
block introduced `david-silver-rl-2015` with a single member, and the script rejects
singleton series); the rows added here already carry their verified seriesId /
seriesTitle / seriesOrder values. Final merged totals: 1,252 resources; README
coverage recomputed from live JSON: 128 / 209 / 133 / 213 / 499 / 70.
